...
class Spacing(object):
    __slots__ = ['_vertical', '_horizontal']
...
class Legend(object):
    ...
    __slots__ = ['_direction', '_fat', '_thickness', '_stretch']
    ...
    def _setdirection(self, value):
        ...
        if not value.lower() in ['vertical', 'horizontal']:
            raise ValueError(
                'direction attribute must be either "vertical" or "horizontal"')
...
class Multi(object):
    ...
    def _settemplate(self, value):
        if isinstance(value, vcs.template.P):
            ...
            if value not in self.x.listelements('template'):
                raise ValueError(value + ' is not an existing template')
            self._template = value
        else:
            raise ValueError(
                'template must be an existing template name or a template')
    template = property(
        _gettemplate,
        _settemplate,
        None,
        "'model' vcs template to use for generating templates")
    ...
    def get(self, column=None, row=None, legend=None,
            font=True, fontlimit=0.8, tname=None):
        """
        ...
        if column >= self.columns:
            raise ValueError(
                'You requested template for column %d but you defined %d columns only' %
                (column, self.columns))
        if row >= self.rows:
            raise ValueError(
                'You requested template for row %d but you defined %d rows only' %
                (row, self.rows))
        ...
        # X width
        dx = (1. - self.margins.right - self.margins.left -
              (self.columns - 1) * self.spacing.horizontal) / self.columns
        ...
        # Y width
        dy = (1. - self.margins.top - self.margins.bottom -
              (self.rows - 1) * self.spacing.vertical) / self.rows

        # Start/End of ys (data area)
        y1 = 1. - self.margins.top - row * \
            (dy + self.spacing.vertical) - dy + yl
        y2 = y1 + dy - yl
    ...
    def preview(self, out='EZTemplate_Multi', bg=1):
        ...
        for i in tmpl:
            ...
            ln_tmp = self.x.createline('_EZTM' + str(i))
            ln_tmp.x = [t.data.x1, t.data.x2, t.data.x2, t.data.x1, t.data.x1]
            ln_tmp.y = [t.data.y1, t.data.y1, t.data.y2, t.data.y2, t.data.y1]
            ln_tmp.type = 'dot'
            self.x.plot(ln_tmp, bg=bg)
            ...
            ln_tmp = self.x.createline('_EZTMleg' + str(i))
            ln_tmp.x = [
                t.legend.x1,
                t.legend.x2,
                t.legend.x2,
            ...
            ln_tmp.y = [
                ...
                t.legend.y1]
            ln_tmp.type = 'dot'
            ln_tmp.priority = t.legend.priority
            self.x.plot(ln_tmp, bg=bg)
            if tmpl == list(range(n)):
                ic += 1
                if ic == self.columns:
