...
class VCSaddon(object):
    ...
    def __init__(self, name=None, source='default', x=None, template=None):
        self._saves = {}
        self.g_nslabs = 1
        if self.g_name not in list(gms.keys()):
            gms[self.g_name] = {}
        if name is None:
            cont = True
            while cont:
                ...
                if nm not in list(gms[self.g_name].keys()):
                    name = nm
                    cont = False
        ...
        if name in list(gms[self.g_name].keys()):
        ...
        if source == 'default':
            ...
            if isinstance(source, str):
                gm = gms[self.g_name].get(source, None)
                if gm is None:
                    raise "error could not find graphic method %s (of type %s)" % (
                        source, self.g_name)
            else:
                gm = source
    ...
    def plot_internal(self, slab=None, slab2=None,
                      template=None, bg=0, x=None, **kwargs):
        """
    ...
    def restore(self, cleanup=True):
        for att in list(self._saves.keys()):
    ...
    def getgm(self, source="default"):
        gm = None
        for nm in list(gms[self.g_name].keys()):
            if source == nm:
                return gms[self.g_name][nm]

        if gm is None:
            raise "Could not find graphic method %s named: %s" % (
                self.g_name, source)

    def creategm(self, name, source='default'):
        return self.__init__(name, source=source, x=self.x,
