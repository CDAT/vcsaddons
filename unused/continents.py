...
class Guc(VCSaddon):
    ...
    def gen_cont_file(self, lon1, lon2, lat1, lat2, type, patch=False):
        ...
        if not (0 <= itype < len(self.ngdc_names)):
            ...
            raise Exception(
                "Error currently only ngdc source supported are: %s" %
                oktype)
        ...
        if found:
            ...
            fnm = "mycont_C%i_L%i_L%i_l%i_l%i.dat" % (
                itype + 1, int(lon1) - 1, int(lon2) + 1, int(lat1) - 1, int(lat2) + 1)
        ...
        if not os.path.exists(fnm):
            ...
            #"Accept": "text/plain"}
            conn = http.client.HTTPConnection("rimmer.ngdc.noaa.gov:80")
            conn.request(
                "POST",
                "/cgi-bin/mgg/coast/get_coast.pl",
                params,
                headers)
            response = conn.getresponse()
            if response.status != http.client.OK:
                raise Exception(
                    "HTTP Request failed (%s), try again later" %
                    (response.reason))
            data = response.read()
...
# fnm=fnm[:-4]
            ...
            for s in sp:
                ...
                while nprocessed < len(tmpsp):
                    ...
                    if n > N:
                        N = n
                    print(n, 1, sp2[1::2].min(), sp2[1::2].max(),
                          sp2[::2].min(), sp2[::2].max(), file=f)
                    for i in range(0, n, 8):
                        vals = sp2[i:i + 8].tolist()
                        for i in range(len(vals) / 2):
                            v1, v2 = vals[i * 2], vals[i * 2 + 1]
                            print("%f %f" % (v2, v1), file=f)
            print("-99 -99", file=f)
            f.close()
            if self.HAVE_BZ2:
                import bz2
